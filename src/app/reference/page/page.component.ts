...
import { Alien, GameSelection, Game } from "../../../data/types";
import { groupItems, IGroupedItems } from "../../../data/groupItems";
...
export class AlienReferencePageComponent implements OnInit {
  ...
  public ngOnInit() {
    // Set defaults
    this.levels = this.Storage.get(STORAGE_PREFIX + "levels") || [
      true,
      true,
      true,
    ];
    this.games = this.Storage.get(STORAGE_PREFIX + "games") || {
      [Game.Encounter]: true,
    };
    this.Aliens.init$.subscribe(() => {
      this.refresh();
    });
  }
  ...
  private refresh() {
    this.groups = groupItems(
      this.Aliens.getMatchingNames(this.levels, this.games).map(
        this.Aliens.get,
      ),
      ["game", "level"],
      ["name"],
